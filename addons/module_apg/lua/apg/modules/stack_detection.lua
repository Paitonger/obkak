...
function APG.checkStack( ent, pcount )
    ...
    if count >= (pcount or max_count) then
        ...
        --    APG.notify("Do not try to crash the server!", ply, 1)
            ...
            local msg = owner:Nick().." ["..owner:SteamID().."]" .. " пытается поставить несколько пропов в одной точке!"
            APG.notify(msg, "admins", 2)
            APG.notify("Нельзя ставить несколько пропов слишком близко друг к другу!", owner, 1)
        end
    end
end
...
APG.hookRegister(mod, "PlayerSpawnedProp","APG_stackCheck",function(ply, model, ent)
	timer.Simple(0.5, function()
    APG.checkStack( ent )
	end)
end)

APG.hookRegister(mod, "CanTool", "APG_stackCheck", function( ply, tr, toolname, tool, button )
   if (toolname ~= "precision" and toolname ~= "stacker" ) then return end
   if IsValid( tr.Entity ) and tr.Entity:GetClass() == "prop_physics" then
      timer.Simple(2, function()
        if not IsValid( tr.Entity ) then return end
        APG.checkStack( tr.Entity )
      end)
   end
end )
