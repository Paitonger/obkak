...
function tttChargeSpeed(ply)
	if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "swat_shield" then
...
function chargeHit(ent,ply)
	if ent:IsPlayer() or ent:IsNPC() then
		if SERVER and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "swat_shield" then
...
function shouldBeBlocked(target,dmginfo)
	if target:IsValid() and target:IsPlayer() and IsValid(target:GetActiveWeapon()) and target:GetActiveWeapon():GetClass() == "swat_shield" then
...
function SWEP:Initialize()
	...
	if table.Count(ents.FindByClass("swat_shield")) <= 1 then
...
function SWEP:OnRemove()
	if table.Count(ents.FindByClass("swat_shield")) <= 0 then
