...
function PatternKeypad.checkVersion()
    http.Fetch("https://raw.githubusercontent.com/Metamist/garrysmod-MetaAddons/master/PatternKeypads/versions.txt", function(body, len, headers, code)
        if code ~= 200 then
        ...
        local currentVersion = parseVersion(PatternKeypad.version)
        ...
        if compareVersions(currentVersion, latestVersion) < 0 then -- Out of date
            ...
            if #changelog > 0 then
                print()
                PatternKeypad.print("A new version is available (",
                    Color(0, 255, 0), latestVersion,
                    Color(255, 255, 255), "), current version is ",
                    Color(255, 0, 0), currentVersion)

                PatternKeypad.print("Please go to ScriptFodder and download the new version!")

                PatternKeypad.print("Changelog: ")

                for i = 1, #changelog do
                    PatternKeypad.print(" * " .. changelog[i])
                end
                print()
            end
        end
...
hook.Add("Initialize", "PKeypad_VersionCheck", function()
    timer.Simple(5, function()
        PatternKeypad.checkVersion()
    end)
end)
