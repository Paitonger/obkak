...
function PANEL:Init()
	...
	local b = functions:Add('DButton','choose button')
		...
		b.DoClick= function()
			...
			LocalPlayer():ChatPrint('Выбери нужную модель до 10 мегабайт и скопируй ссылку на нее')
            
            local frame = vgui.Create("DFrame")
            frame:SetSize(300, 200)
            frame:Center()
            frame:SetTitle('Скопируй ссылку на модель из стим браузера')
            frame:MakePopup()
            
            local mdl = vgui.Create( "DTextEntry", frame )
            mdl:Dock( TOP )
            mdl:SetPlaceholderText( "Ссылка на модель до 10 мегабайт" )
            ...
            button.DoClick = function()
                local link = mdl:GetValue()
                link = string.Replace(link, "https://steamcommunity.com/sharedfiles/filedetails/", "")
                link = string.Replace(link, "?id=", "&")
                link = string.Explode("&", link)
                UIChoseWorkshop(link[2], self) 
            end
