...
// 
// DarkRP
CreateConVar( "cuffs_restrictwarrant", 0, {FCVAR_ARCHIVE,FCVAR_SERVER_CAN_EXECUTE,FCVAR_REPLICATED} )
hook.Add( "canRequestWarrant", "Cuffs PreventWarrant", function( crim, cop, reason )
	if cvars.Bool("cuffs_restrictwarrant") and cop:IsHandcuffed() then return false,"Ты не можешь это сделать в наручниках!" end
end)
...
hook.Add( "canArrest", "Cuffs RestrictArrest", function( cop, crim ) // DarkRP Arrest hook
    if crim:getDarkRPVar("Arrested") then return true end
	if IsValid(crim) and cvars.Bool("cuffs_restrictarrest") and not crim:IsHandcuffed() then return false,"Ты должен надеть на него наручники!" end
end)
...
net.Receive( "Cuffs_TiePlayers", function(_,ply)
	...
	-- if not tr.HitWorld and tr.Entity:GetPhysicsObject():IsMotionEnabled() == true then return end
