...
function plogs.OpenMenu(pl)
    if not plogs.data then return end
	local c = 0
	for k, v in pairs(plogs.data) do
		timer.Simple(0.3 * c, function()
			if IsValid(pl) then
...
function plogs.Log(type, str, copy, noDiscord)
    
    if not noDiscord then
        DiscordLog({
            channel = type,
            content = str,
        })
