...
if SERVER then 
	...
	resource.AddFile( "resource/fonts/dimaets76561197999799284.ttf" )
	return 
end
...
function flatHUD.draw()
	...
	/*---------------------------------------------------------------------------
		Variable display
	---------------------------------------------------------------------------*/
	draw.SimpleText( LocalPlayer().DarkRPVars.rpname, "flatHUD_font28", x + 120 * config.scale, y + 5 * config.scale, Color( 240, 240, 240 ), TEXT_ALIGN_TOP, TEXT_ALIGN_LEFT )
	draw.SimpleText( LocalPlayer().DarkRPVars.job, "flatHUD_font18", x + 140 * config.scale, y + 35 * config.scale, Color( 240, 240, 240 ), TEXT_ALIGN_TOP, TEXT_ALIGN_LEFT )
	draw.SimpleText( "$" .. formatCurrency( math.Round( money ) ), "flatHUD_font18", x + 140 * config.scale, y + 60 * config.scale, Color( 240, 240, 240 ), TEXT_ALIGN_TOP, TEXT_ALIGN_LEFT )
	draw.SimpleText( "$" .. LocalPlayer().DarkRPVars.salary, "flatHUD_font18", x + 140 * config.scale, y + 85 * config.scale, Color( 240, 240, 240 ), TEXT_ALIGN_TOP, TEXT_ALIGN_LEFT )
	...
	if GetGlobalBool("LockDown1") then
	    local text = "Комендантский час"
        draw.SimpleText( text, "chelog-ib-shadow", ScrW() / 2, ScrH() - ScrH() + 40, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( text, "chelog-ib", ScrW() / 2, ScrH() - ScrH() + 40, Color( 255, 0, 0 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Причина: "..GetGlobalString("ReasonLockDown"), "Trebuchet24", ScrW() / 2, ScrH() - ScrH() + 75, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end --chelog-ib.small
