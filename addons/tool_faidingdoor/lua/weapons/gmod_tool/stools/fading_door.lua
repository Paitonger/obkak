...
local function fadeToggleActive(self, ply)
	if noKeyboard:GetBool() and not numpad.FromButton() and ply:Team() ~= TEAM_ADMIN then
		ply:ChatPrint("Нельзя активировать Fading Door с клавиатуры.")
		ply:ChatPrint("Используй кейпад или кнопку для его активации.")
		return
	end
