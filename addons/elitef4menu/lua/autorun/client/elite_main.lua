...
table.insert( sideButtons, { Title = "Информация", Func = function() LoadCommandPanel() end } )
...
local function BuildAdminMenu()
	...
	BG_PANEL.Paint = function( self, w, h )
		...
		surface.SetMaterial( Material( "icon16/emoticon_grin.png" ) )
		surface.DrawTexturedRect( 16, 16, 12, 12 )
		surface.SetMaterial( Material( "icon16/emoticon_tongue.png" ) )
		surface.DrawTexturedRect( 16 + 12 + 4, 16, 12, 12 )
		surface.SetMaterial( Material( "icon16/emoticon_wink.png" ) )
	...
	BG_SIDEBAR.Paint = function( self, w, h )
		draw.RoundedBoxEx( 4, 0, 0, w, h,  Color( 47, 52, 57 ), false, false, true, false )
	end
	...
	BG_SIDEHEAD.Paint = function( self, w, h )
		draw.RoundedBox( 0, 0, 0, w, h, Color(236, 113, 71 ) )
	
		draw.RoundedBox( 0, 12, (75 / 2) - 19, 36, 36, Color( 60, 60, 60 ) )
	...
	local CHECKBOX_THEME = vgui.Create("DCheckBoxLabel", BG_SIDEBAR)
	CHECKBOX_THEME:DockMargin(40,0,0,30)
	CHECKBOX_THEME:Dock(BOTTOM)
	CHECKBOX_THEME:SetText('Темная тема')
	CHECKBOX_THEME:SetConVar("wayskin_enable")
	
	local CHECKBOX_MODEL = vgui.Create("DCheckBoxLabel", BG_SIDEBAR)
	CHECKBOX_MODEL:DockMargin(40,0,0,30)
	CHECKBOX_MODEL:Dock(BOTTOM)
	CHECKBOX_MODEL:SetText('Кастомные скины')
	CHECKBOX_MODEL:SetConVar("outfitter_enabled")
	...
	for k, v in pairs( sideButtons ) do
	    if v.Title == 'Оружие' and LocalPlayer():Team() ~= TEAM_GUN then continue end
		local SIDE_BUTTON = vgui.Create( "DButton", BG_SIDELIST )
		SIDE_BUTTON:SetSize( 182, 60 )
		SIDE_BUTTON:SetText( v.Title )
		if v.Title == 'Информация' then
		   SIDE_BUTTON:SetTextColor( Color( 236, 113, 73 ) )
		else
		...
		SIDE_BUTTON.Paint = function( self, w, h )
			if self.hover then
				draw.RoundedBox( 0, 0, 0, w, h, Color( 35, 40, 40 ) )
			else
				draw.RoundedBox( 0, 0, 0, w, h, Color( 47, 52, 57 ) )
			end
		end
		...
		SIDE_BUTTON.OnCursorExited = function( self )
			self.hover = false
    		if v.Title == 'Информация' then
    		   self:SetTextColor( Color( 236, 113, 73 ) )
    		else
