...
local function PopulateJobList()
	for k, v in pairs( RPExtraTeams ) do
		...
		if show then
			...
			if v.max == team.NumPlayers(v.team) and v.max > 0 then
			jobButton:SetText( "Занято")
			jobButton:SetFont( "Trebuchet24" )
			jobButton:SetTextColor( Color( 60, 60, 60 ) )
		    else
		    jobButton:SetText( "Стать")
			jobButton:SetFont( "Trebuchet24" )
				...
				if type(v.model) == "table" and table.Count( v.model ) > 1 then
					...
					if v.vote then
						if ((v.admin == 0 and LocalPlayer():IsAdmin()) or (v.admin == 1 and LocalPlayer():IsSuperAdmin())) then
								...
								menu:AddOption("Голосование", function() RunCmd("/vote"..v.command) BG_PANEL:Remove() end)
