...
hook.Add("HUDPaint", "Tazer", function()
	...
	if STUNGUN.Thirdperson and STUNGUN.AllowSwitchFromToThirdperson and IsValid(LocalPlayer():GetNWEntity("tazerviewrag")) then
		local txt = string.format("Нажми %s чтобы изменить вид на первое/третье лицо.", input.LookupBinding("+duck"))
...
local bolt1 = Material("icon16/lightning.png")
local bolt1_o = Material("icon16/lightning.png")
local bolt1_g = Material("icon16/lightning.png") -- accept
local bolt2 = Material("icon16/lightning.png")
function SWEP:DrawScreen(x, y, w, h)
	...
	for k,v in pairs(boltpositions) do
		...
		surface.SetDrawColor(Color(255,255,255,a + 1))
	...
	surface.SetDrawColor(Color(255,255,255,math.cos(glowtimer / 40 + math.pi) * 50 + 50))
	surface.SetMaterial(bolt1_g)
	surface.DrawTexturedRect(bx-2, by-32, 32, 32)
	*/
end
