...
function meta:SetupInventory()
	...
	-- delay loading and try to refresh the player's inventory sizes based on a configurable delay
	timer.Simple( itemstore.config.LoadDelay, function()
		if not IsValid( self ) then return end
		...
		local inv_size = itemstore.config.InventorySizes[ usergroup ] or itemstore.config.InventorySizes[ "default" ]
		local bank_size = itemstore.config.BankSizes[ usergroup ] or itemstore.config.BankSizes[ "default" ]

		self.Inventory:SetSize( unpack( inv_size ) )
		self.Bank:SetSize( unpack( bank_size ) )
...
function meta:PickupItem( ent )
	...
	if not IsValid( ent ) or ent.__Deleted or ent:IsMarkedForDeletion() or not itemstore.items.Pickups[ ent:GetClass() ] then
		return false end
	...
	if slot then
		itemstore.Log( "{1} picked up a {2} (from a {3} entity) into their inventory", self, item, ent )
		hook.Call( "ItemStoreItemPickedUp", GAMEMODE, self, item, ent )
...
function meta:MoveItem( from_con_id, from_slot, to_con_id, to_slot )
	...
	if from_con:CanWrite( self, "move", to_con, to_slot, to_item, from_slot, from_item ) ~= false and
		...
		if itemstore.config.VerboseLogging then
			itemstore.Log( "{1} swapped a(n) {2} item and a {3} item", self, from_item, to_item )
		end

		hook.Call( "ItemStoreItemsMoved", GAMEMODE, self, from_con, from_slot, from_item, to_con, to_slot, to_item )
	end
end
...
function meta:UseItem( con_id, slot, ... )
	...
	itemstore.Log( "{1} used a {2} item", self, item )
	hook.Call( "ItemStoreItemUsed", GAMEMODE, self, con, item )
end
...
function meta:UseItemWith( from_con_id, from_slot, to_con_id, to_slot )
	...
	itemstore.Log( "{1} used a {2} item with a {3} item", self, from_item, to_item )
	hook.Call( "ItemStoreItemUsedWith", GAMEMODE, self, from_con, from_slot, from_item, to_con_to_slot, to_item )
end
...
function meta:DropItem( con_id, slot )
	...
	local tr = util.QuickTrace( self:GetShootPos(),
		self:GetAimVector() * itemstore.config.DropDistance, self )
	...
	itemstore.Log( "{1} dropped a {2} item", self, item )
	hook.Call( "ItemStoreItemDropped", GAMEMODE, self, con, slot, item )
end
...
function meta:DestroyItem( con_id, slot )
	...
	if item and con:CanWrite( self, "destroy", slot, item ) ~= false then
		...
		itemstore.Log( "{1} destroyed a {2} item", self, item )
		hook.Call( "ItemStoreItemDestroyed", GAMEMODE, self, con, slot, item )
	end
end
...
function meta:MergeItem( from_con_id, from_slot, to_con_id, to_slot )
	...
	if not ( from_con == to_con and from_slot == to_slot ) and
		...
		to_item:CanMerge( from_item ) then
			...
			itemstore.Log( "{1} merged a {2} item and a {3} item", self, from_item, to_item )
			hook.Call( "ItemStoreItemMerged", GAMEMODE, self, from_con, from_slot, from_item, to_con_to_slot, to_item )
	end
end
...
function meta:SplitItem( con_id, slot, amount )
	...
	if con:FirstEmptySlot() and item:CanSplit( amount ) and con:CanWrite( self, "split", slot, item, amount ) ~= false then
		local stack = item:Split( amount )
		con:AddItem( stack, true )

		itemstore.Log( "{1} split a {2} item stack with a quantity of {3}", self, item, amount )
		hook.Call( "ItemStoreItemDropped", GAMEMODE, self, con, slot, amount, item, stack )
	end
end
...
hook.Add( "PlayerUse", "ItemStorePickup", function( pl, ent )
	...
	if ent.__Deleted or ent:IsMarkedForDeletion() then
...
hook.Add( "PlayerDeath", "ItemStoreDeathLoot", function( pl )
	...
	ent:SetPos( pl:GetPos() + Vector( 0, 0, 25 ) )
	ent:Spawn()
...
util.AddNetworkString( "ItemStoreRecreateContextMenu" )
concommand.Add( "itemstore_refreshinventories", function( pl, cmd, args )
	if IsValid( pl ) and not pl:IsSuperAdmin() then return end

	for _, pl in ipairs( player.GetAll() ) do
		if IsValid( pl ) then
			local usergroup = pl:GetUserGroup()
			...
			if pl.Inventory then pl.Inventory:SetSize( unpack( inv_size ) ) end
			if pl.Bank then pl.Bank:SetSize( unpack( bank_size ) ) end

			net.Start( "ItemStoreRecreateContextMenu" )
			net.Send( pl )
		end
	end
end )
