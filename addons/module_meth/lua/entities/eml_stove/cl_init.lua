...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos+ang:Up()*14.5, ang, 0.1)
			...
			if self:GetNWInt("gasStorage")*100 == 0 then
			draw.SimpleTextOutlined("Заправте печь газом!", "methFont1", -211, -48, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT, 1, Color(25, 25, 25, 200));
			else
			draw.SimpleTextOutlined("Газ: "..math.Round((self:GetNWInt("gasStorage")*100)/self:GetNWInt("gasStorageMax")).."%", "methFont1", -211, -48, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT, 1, Color(25, 25, 25, 200));
			end
