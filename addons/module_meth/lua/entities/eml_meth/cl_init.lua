...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos + ang:Up(), Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.1)
				draw.SimpleTextOutlined("Кристализированный мет ("..self:GetNWInt("amount").." lbs)", "methFont", 8, -96, Color(1, 241, 249, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
		cam.End3D2D();
	end;
end;
