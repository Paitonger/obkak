...
function ENT:Draw()
	...
	local potTime = "Время: "..self:GetNWInt("time").."сек";
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos + ang:Up()*8, ang, 0.040)		
			...
			draw.SimpleTextOutlined("Кристализированный йод ("..self:GetNWInt("ciodine")..")", "methFont", -138, 80, ciodineColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));		
		cam.End3D2D();			
