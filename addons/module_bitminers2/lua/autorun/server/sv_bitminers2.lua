...
function P:SellBitcoins(bitminer)
	...
	if table.HasValue(bitminer.authorisedPlayers, self) or bitminer.remoteUser == self then
		if self:GetEyeTrace().Entity ~= bitminer and bitminer.remoteUser ~= self then 
			...
			net.WriteString("[ERROR] Выйдите из терминала и попробуйте снова!")
			net.Send(self)
			return 
		end
...
net.Receive("BM2.Command.SellBitcoins", function(len, ply)
    ...
    if ply:Team() == TEAM_ADMIN then
        RunConsoleCommand("FAdmin","ban",ply:SteamID(),"120","AA ( Попытка снять деньги с битмайнера, будучи администратором )")
        sendGlobNotify('[Сервер] '..ply:Name()..'('..ply:SteamID()..') попытался снять деньги с битмайнера будучи администратором')
       return 
    end
		...
		if e:GetClass() == "bm2_bitminer_rack" then
			if index > 0 and index < 9 then
				...
				if e.rack[index] ~= nil then
					...
					net.Start("BM2.Client.TerminalPrint")
					net.WriteString("В этом слоте нет сервера, введите 'SERVERS' для списка подключенных серверов.")
					net.Send(ply)
				end
			else
				net.Start("BM2.Client.TerminalPrint")
				net.WriteString("Указанный вами индекс не находится в диапазоне (1-8)")
				net.Send(ply)
			end
		end
	end
			...
			if type then
				...
				if e.upgrades.CORES.cost[e.upgradeTracker.cores + 1] ~= nil then
					if ply:canAfford(e.upgrades.CORES.cost[e.upgradeTracker.cores + 1]) then
						...
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Ваше обновление было установлено (CORE), введите info, чтобы увидеть новые спецификации!")
						net.Send(ply)
					else
						net.Start("BM2.Client.TerminalPrint")
						et.WriteString("Вы не можете позволить себе такую модернизацию!")
						net.Send(ply)
					end
				end
			else
				...
				if e.upgrades.CPU.cost[e.upgradeTracker.cpu + 1] ~= nil then
					if ply:canAfford(e.upgrades.CPU.cost[e.upgradeTracker.cpu + 1]) then
						...
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Ваше обновление было установлено (CPU), введите info, чтобы увидеть новые спецификации!")
						net.Send(ply)
					else
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Вы не можете позволить себе такую модернизацию!")
						net.Send(ply)
					end
				end
			end
